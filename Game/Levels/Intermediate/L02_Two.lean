...
In Lean we use the symbol λ (`\\lambda`) or `fun` to describe a function by placing the name of the variable
after `λ` or `fun`.
So `λ x => x^3 + 7` defines the function which takes input `x` and outputs `x^3 + 7` in Lean.
Alternatively, `fun x => x^3 + 7` denotes the same function.
...
Statement : Continuous (λ (x : ℝ) => 5 * x ^ 2 + x + 6) := by
...
LemmaDoc Continuous.pow as "Continuous.pow" in "Topology"
  "The power of a continous function is continuous."
